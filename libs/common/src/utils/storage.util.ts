...
export type StorageCallback<T = any> = (newValue: T, oldValue: T) => void;
 ...
 * returned.
 */
export async function getStorage(key: string, defaultValue?: unknown) {
  try {
 ...
 */
export function setStorage(key: string, val: unknown) {
  try {
    return browser.storage.sync.set({ [key]: val });
  } catch (e) {
    console.log(`[Error in setStorage]: ${e}`);
    throw e;
  }
}
